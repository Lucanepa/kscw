...
- **The sync steps no longer open on the last run's finished progress bar.** Clicking a step queued the job, but the dispatchers pick work up on a one-minute cron — and for that minute the window showed the *previous* run's phase, its 100% bar and its whole log, so a sync you had just asked for opened reading "Synced from ClubDesk · 100%" two seconds in. The progress is cleared the moment the job is queued now. Log lines also carry the Swiss date and time (`08.09.2026 22:48:26`) instead of a bare UTC clock that read two hours behind with no day on it.
